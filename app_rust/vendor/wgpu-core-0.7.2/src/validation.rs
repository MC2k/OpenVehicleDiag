...
enum Varying {
    ...
    // Retain shader built-in metadata even though validation currently skips it.
    #[allow(dead_code)]
    BuiltIn(naga::BuiltIn),
}

// Retain the upstream representation for shader specialization metadata.
#[allow(dead_code)]
#[derive(Debug)]
...
struct EntryPoint {
    ...
    // Preserve specialization metadata without adding new validation behavior.
    #[allow(dead_code)]
